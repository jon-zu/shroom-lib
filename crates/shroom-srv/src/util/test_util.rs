...
use super::clock::{GameClock, GameClockRef};
...
impl MockMsg {
    pub fn try_from_bytes(b: &[u8]) -> anyhow::Result<Self> {
        ...
        match op {
            ...
            1 => Ok(Self::Sub(v)),
            2 => Ok(Self::Set(v)),
            _ => anyhow::bail!("Invalid opcode: {op} - {b:?}"),
        }
    }
...
impl Context for MockCtx {
    ...
    fn time(&self) -> super::clock::GameTime {
        self.clock.time()
    }
...
pub fn test_clock() -> GameClockRef {
