import binascii
import os

def hex_array_to_binary(hex_array, output_file):
    # Combine the lines and remove line breaks, spaces, prefixes, and commas
    hex_values = ''.join(hex_array.split()).replace('0x', '').replace(',', '')

    # Convert hex string to binary
    binary_data = binascii.unhexlify(hex_values)

    # Write the binary data to a file
    with open(output_file, 'wb') as file:
        file.write(binary_data)

shuffle = """
		"""

ig_seed = "0xF2, 0x53, 0x50, 0xC6"
aes_key = """
		0x13, 0x00, 0x00, 0x00,
		0x08, 0x00, 0x00, 0x00,
		0x06, 0x00, 0x00, 0x00,
		0xB4, 0x00, 0x00, 0x00,
		0x1B, 0x00, 0x00, 0x00,
		0x0F, 0x00, 0x00, 0x00,
		0x33, 0x00, 0x00, 0x00,
		0x52, 0x00, 0x00, 0x00,"""

path = "crates/shroom-crypto/src/default_keys"
hex_array_to_binary(shuffle, os.path.join(path, "ig_shuffle.bin"))
hex_array_to_binary(ig_seed, os.path.join(path, "ig_seed.bin"))
hex_array_to_binary(aes_key, os.path.join(path, "net/aes_key.bin"))