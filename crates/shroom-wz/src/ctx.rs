...
use crate::{cipher::WzCipher, ty::WzStr};
...
pub struct WzContext<'a>(pub &'a WzCipher);

impl<'a> Deref for WzContext<'a> {
    type Target = WzCipher;
...
impl<'a> WzContext<'a> {
    pub fn new(crypto: &'a WzCipher) -> Self {
        Self(crypto)
    }
}
...
impl<'a> WzImgReadCtx<'a> {
    pub fn new(crypto: &'a WzCipher, str_table: &'a WzStrTable) -> Self {
...
impl<'a> WzImgWriteCtx<'a> {
    pub fn new(crypto: &'a WzCipher, str_table: &'a WzStrWriteTable) -> Self {
        Self { crypto, str_table }
    }
}
